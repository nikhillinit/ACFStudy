class VideoPlayer {
    ...
    saveNotes() {
        const notes = this.container.querySelector('.notes-textarea').value;
        const lectureId = this.currentVideo?.id;
        
        if (lectureId && notes.trim()) {
            // Save to localStorage for demo purposes
            localStorage.setItem(`notes_${lectureId}`, notes);
    ...
    loadNotes(lectureId) {
        ...
        this.container.querySelector('.notes-textarea').value = savedNotes;
    }

    toggleTranscript() {
        const transcriptPanel = this.container.querySelector('.transcript-panel');
        const toggleBtn = this.container.querySelector('.toggle-transcript');
    ...
    updateProgress(lectureId) {
        ...
        const progress = this.watchProgress.get(lectureId) || 0;
        const progressDisplay = this.container.querySelector('.progress');
        progressDisplay.textContent = `${Math.round(progress * 100)}% watched`;
    }

    // Playlist methods for external control
    loadPlaylist(playlistId) {
        if (!this.videoLibrary) return;
        
        const playlist = this.videoLibrary.playlists[playlistId];
        if (playlist && playlist.lectures.length > 0) {
            this.loadVideo(playlist.lectures[0]);
        }
    }

    // Demo functionality for progress simulation
    simulateProgress(lectureId, progress) {
        this.watchProgress.set(lectureId, progress);
        this.updateProgress(lectureId);
        
        // Update playlist UI if visible
